<template>
  ...
  >
    <div class="vc-calendar-timetable-wrap">
      <div :class="['vc-calendar-body', `mc-range-mode-${tableMode}`]">
        <div class="vc-calendar-rang-head" v-if="tableMode === 'monthRange'">
          <div class="vc-calendar-rang-year-month-box">
            <span className="vc-calendar-rang-head-year">{{
              formatRangeMonth[0]
            }}</span>
            <span className="vc-calendar-rang-head-month">{{
              formatRangeMonth[1]
            }}</span>
          </div>
        ...
        <div className="vc-calendar-content">
          <div
            v-for="(days, k1) in monthRender.value"
            :key="k1"
              ...
              :class="[
                {
                  ...
                  'vc-calendar-dayoff': k2 === (monFirst ? 0 : 5) || k2 === 6,
              ...
              <div class="vc-calendar-day-container">
                ...
                <div
                  :class="[
                    ...
                    child.tileContent.className
                  ]"
                  v-if="child.tileContent"
                >
                  {{ child.tileContent.content }}
                </div>
                <div class="vc-calendar-remark-text" v-if="child.remark">
                  {{ child.remark }}
                </div>
                <div
...
export default {
  ...
  setup(props: TimeTableInterface, { emit }: any) {
    ...
    const completion = ref(
      propsTableMode.value === "week" || propsCompletion.value
    );
    ...
    function select(k1: any, k2: any, child: any) {
      ...
      switch (selectMode.value) {
        ...
        case "multi":
          ...
          break;
        case "range":
          selectValue = rangeSelect(
            selectDate?.value as { start?: string; end?: string },
            date
          );
          break;
        case "multiRange":
          selectValue = multiRange(
            selectDate?.value as { start?: string; end?: string }[],
            date
          );
          break;
    ...
    function render({ year, month, day, renderer, payload }: any) {
      ...
      if (completion.value) {
        ...
        const [nextYear, nextMonth] = [
          ...
          computedNextMonth(month)
        ];

        if (!Array.isArray(temp[line])) temp[line] = [];

        const lastWeekDayCompletionCount =
          7 - temp[line].length + 7 * completionWeeksCount;
        ...
        temp[0].unshift(
          ...Array.from({ length: firstWeekDayCompletionCount }).fill(
            "PLACEHOLDER"
          )
        );
      }
